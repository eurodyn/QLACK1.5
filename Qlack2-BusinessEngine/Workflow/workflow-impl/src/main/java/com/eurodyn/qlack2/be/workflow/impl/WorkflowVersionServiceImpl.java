...
public class WorkflowVersionServiceImpl implements WorkflowVersionService {
	...
	public Long countWorkflowVersionsLockedByOtherUser(CountWorkflowVersionsLockedByOtherUserRequest req) {
		...
		return WorkflowVersion.countWorkflowVersionsLockedByOtherUser(em, workflow.getId(), req.getSignedTicket().getUserID());
	}
	...
	@Override
	@ValidateTicket
	public String getLatestFinalizedWorkflowVersionIdByWorkflowId(GetWorkflowVersionIdByNameRequest req) {
		LOGGER.log(Level.FINE, "Getting latest finalized workflow version id with workflow id {0}", req.getWorkflowId());
		...
		return WorkflowVersion.findLatestFinalizedWorkflowVersionByWorkflowId(em,  workflow.getId());
	}
	...
	private void checkConditionValidations(Map<String, ConditionDTO> conditions)
	{
		...
		for (ConditionDTO condition : conditions.values()) {
			...
			if (condition.getParentCondition() != null) {
				...
				cyclicDependencyCheck(conditions, index, condition, checkedCondition);
				index++;
			}
		}
	}

	private void cyclicDependencyCheck(Map<String, ConditionDTO> conditions, int index, ConditionDTO condition,
			ConditionDTO checkedCondition) {
	...
	public void finaliseWorkflowVersion(FinaliseWorkflowVersionRequest req) {
		...
		throwInvalidActionException(processId);

		version.setState(State.FINAL);
		version.setLockedBy(null);
		version.setLockedOn(null);
		version.setEnableTesting(false);
	...
	private void throwInvalidActionException(String processId) {
	...
	private String findProcessIdFromVersionContent(String content)
	{
		...
		try {
	        ...
	        LOGGER.log(Level.INFO, "ProcessId: {0}" , processId);
		}
		catch(Exception e)
		{

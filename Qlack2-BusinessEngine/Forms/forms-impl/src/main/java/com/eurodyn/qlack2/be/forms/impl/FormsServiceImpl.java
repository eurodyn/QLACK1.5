...
public class FormsServiceImpl implements FormsService {
	...
	public void updateForm(UpdateFormRequest request) {
		...
		List<String> oldLocales = new ArrayList<>();
		if (form.getLocales() != null) {
			oldLocales.addAll(form.getLocales());
		}

		List<String> newLocales = new ArrayList<>();
	...
	private void updateTranslations(List<FormVersion> formVersions,
		...
		if (!oldLocales.isEmpty() || !newLocales.isEmpty()) {
			for (FormVersion formVersion : formVersions) {
				...
				GroupDTO groupDTO = groupService.getGroupByName(formVersion.getId());

				if (groupDTO != null) {
					String groupId = groupDTO.getId();

					// Delete translations for removed locales
					deleteTranslationsOldLocales(oldLocales, groupId);

					// Add default translations for added locales
					if (!newLocales.isEmpty()) {
						// Fetch existing keys
						KeySearchCriteriaBuilder builder = KeySearchCriteriaBuilder.createCriteria();
						builder.inGroup(groupId);
						KeySearchCriteria criteria = builder.build();

						List<KeyDTO> keys = keyService.findKeys(criteria, true);

						addDefaultTranslations(newLocales, keys);

					}

					AuditFormVersionDTO auditFormVersionDTO = converterUtil
							.formVersionToAuditFormVersionDetailsDTO(formVersion);
					auditClientService.audit(LEVEL.QBE_FORMS.toString(), EVENT.UPDATE.toString(),
							GROUP.FORM_VERSION.toString(), null, "SYSTEM", auditFormVersionDTO);
				}
			}
		}
	}

	private void deleteTranslationsOldLocales(List<String> oldLocales, String groupId) {
	...
	private void addDefaultTranslations(List<String> newLocales, List<KeyDTO> keys) {
		if (keys != null) {
			for (KeyDTO key : keys) {
				...
				for (String newLocale : newLocales) {
					LanguageDTO language = languageService.getLanguageByLocale(newLocale);

					translations.put(language.getId(), key.getName());
				}

				keyService.updateTranslationsForKey(key.getId(), translations);
			}
		}
	}

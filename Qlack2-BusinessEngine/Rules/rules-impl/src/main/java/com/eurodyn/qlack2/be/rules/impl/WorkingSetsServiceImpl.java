...
public class WorkingSetsServiceImpl implements WorkingSetsService {
	...
	@ValidateTicket
	@Override
	public String getLatestFinalizedWorkingSetVersionIdByWorkingSetName(GetWorkingSetVersionIdByNameRequest request) {
		String projectId = request.getProjectId();
		String workingSetName = request.getWorkingSetName();
		String name = WorkingSetVersion.findLatestFinalizedIdByWorkingSetName(em, projectId, workingSetName);

		LOGGER.log(Level.FINE, "Get latest finalized working set version id by project {0}, working set {1} and name {2}.",
				new Object[]{projectId, workingSetName, name});
	...
	public CanUpdateEnabledForTestingWorkingSetResult canUpdateEnabledForTestingWorkingSetVersion(UpdateWorkingSetVersionRequest request) {
		...
		boolean canBeUpdated = !notTestingRuleVersions.isEmpty();
		canBeUpdated |= !notTestingDataModelVersions.isEmpty();
		canBeUpdated |= !notTestingLibraryVersions.isEmpty();
		if (canBeUpdated) {
	...
	public CanEnableTestingWorkingSetResult canEnableTestingWorkingSetVersion(EnableTestingWorkingSetVersionRequest request) {
		...
		boolean canBeEnabled = !notTestingRuleVersions.isEmpty();
		canBeEnabled |= !notTestingDataModelVersions.isEmpty();
		canBeEnabled |= !notTestingLibraryVersions.isEmpty();
		...
		if (canBeEnabled) {
			...
			addNotTestingVersionNames(notTestingRuleVersions, notTestingDataModelVersions, notTestingLibraryVersions, result);
	...
	private void addNotTestingVersionNames(List<RuleVersion> notTestingRuleVersions,
			List<DataModelVersion> notTestingDataModelVersions, List<LibraryVersion> notTestingLibraryVersions,
			CanEnableTestingWorkingSetResult result) {
		List<String> ruleVersionNames = new ArrayList<>();
		for (RuleVersion ruleVersion : notTestingRuleVersions) {
			ruleVersionNames.add(ruleVersion.getRule().getName() + " / " + ruleVersion.getName());
		}
	...
	public CanFinalizeWorkingSetResult canFinalizeWorkingSetVersion(FinalizeWorkingSetVersionRequest request) {
		...
		boolean canFinalized = !notFinalizedRuleVersions.isEmpty();
		canFinalized |= !notFinalizedDataModelVersions.isEmpty();
		canFinalized |= !notFinalizedLibraryVersions.isEmpty();
		...
		if (canFinalized) {
			...
			addNotFinalizedVersionNames(notFinalizedRuleVersions, notFinalizedDataModelVersions,
					notFinalizedLibraryVersions, result);
	...
	private void addNotFinalizedVersionNames(List<RuleVersion> notFinalizedRuleVersions,
			List<DataModelVersion> notFinalizedDataModelVersions, List<LibraryVersion> notFinalizedLibraryVersions,
			CanFinalizeWorkingSetResult result) {
		List<String> ruleVersionNames = new ArrayList<>();
	...
	public void enableTestingWorkingSetVersion(EnableTestingWorkingSetVersionRequest request) {
		...
		if (enableTesting) {
			...
			List<String> dataModelVersionIds = loadContainedResources(ruleVersions, dataModelVersions, libraryVersions);
	...
	private List<String> loadContainedResources(List<RuleVersion> ruleVersions,
			List<DataModelVersion> dataModelVersions, List<LibraryVersion> libraryVersions) {
		List<String> ruleVersionIds = new ArrayList<>();
		for (RuleVersion ruleVersion : ruleVersions) {
			ruleVersionIds.add(ruleVersion.getId());
		}
	...
	private void checkCanViewRules(SignedTicket ticket, String versionId) {
		List<RuleVersion> ruleVersions = RuleVersion.findByWorkingSetVersionId(em, versionId);
		for (RuleVersion ruleVersion : ruleVersions) {
			Rule rule = ruleVersion.getRule();
	...
	private void checkCanViewDataModels(SignedTicket ticket, String versionId)  {
		List<DataModelVersion> dataModelVersions = DataModelVersion.findByWorkingSetVersionId(em, versionId);
		for (DataModelVersion dataModelVersion : dataModelVersions) {
			DataModel dataModel = dataModelVersion.getDataModel();
			if (!securityUtils.canViewDataModel(ticket, dataModel)) {
	...
	private void checkCanViewLibraries(SignedTicket ticket, String versionId) {
		List<LibraryVersion> libraryVersions = LibraryVersion.findByWorkingSetVersionId(em, versionId);
		for (LibraryVersion libraryVersion : libraryVersions) {
			Library library = libraryVersion.getLibrary();

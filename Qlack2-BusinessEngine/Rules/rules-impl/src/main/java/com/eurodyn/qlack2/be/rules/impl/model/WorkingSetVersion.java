...
public class WorkingSetVersion implements Serializable {
	...
	public static final String findLatestFinalizedIdByWorkingSetName(EntityManager em, String projectId, String workingSetName) {
		String sql =
				"SELECT name FROM (SELECT v.name FROM RUL_WORKING_SET_VERSION v "
				+ "JOIN rul_working_set s ON s.id = v.working_set_id "
				+ "WHERE s.project_Id = :projectId AND s.name = :workingSetName AND v.state = 1 order by v.last_modified_on desc) WHERE rownum = 1";
		try {

...
public class RuleVersion implements Serializable {
	...
	public static final String findLatestFinalizedIdByRuleName(EntityManager em, String projectId, String ruleName) {
		String jpql =
				"SELECT name FROM (SELECT v.name FROM RUL_RULE_VERSION v JOIN rul_rule rule on rule.id = v.rule_id WHERE rule.project_Id = :projectId "
				+ "AND rule.name = :ruleName AND v.state = 1 order by v.last_modified_on desc) WHERE rownum = 1";

			return (String) em.createNativeQuery(jpql)
					.setParameter("projectId", projectId)
					.setParameter("ruleName", ruleName)
					.getSingleResult();
	}

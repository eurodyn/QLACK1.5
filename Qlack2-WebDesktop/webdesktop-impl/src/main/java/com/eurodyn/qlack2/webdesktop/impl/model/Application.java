...
import com.eurodyn.qlack2.webdesktop.api.dto.ApplicationInfo.DISPLAY_ON;
...
public class Application implements Serializable {
	...
	public static List<Application> getAppsForGroup(String groupKey, Boolean active, EntityManager em) {
		CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Application> cq = cb.createQuery(Application.class);
        Root<Application> root = cq.from(Application.class);
        cq = cq.select(root);
        cq = cq.where(cb.and(cb.isNotNull(root.<String>get("bundleSymbolicName")), cb.equal(root.get("groupKey"), groupKey)));
        if (active != null) {
            Predicate pr = cb.equal(root.get("active"), active);
            cq = cq.where(cb.and(cq.getRestriction(), pr));
        }
	...
	public static List<Application> getAppsWithNoGroup(Boolean active, EntityManager em) {
		CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Application> cq = cb.createQuery(Application.class);
        Root<Application> root = cq.from(Application.class);
        cq = cq.select(root);
        cq = cq.where(cb.and(cb.isNotNull(root.<String>get("bundleSymbolicName")), cb.equal(root.get("displayOn"), DISPLAY_ON.item)));
        if (active != null) {
            Predicate pr = cb.equal(root.get("active"), active);
            cq = cq.where(cb.and(cq.getRestriction(), pr));
        }

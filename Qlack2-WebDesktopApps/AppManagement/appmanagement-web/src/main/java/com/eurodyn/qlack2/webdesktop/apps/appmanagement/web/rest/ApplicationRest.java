...
public class ApplicationRest {
	...
	public List<TreeGroupRDTO> getApplicationsAsTree() {
		...
		List<TreeGroupRDTO> appGroups = new ArrayList<TreeGroupRDTO>(groupKeys.size() + 1); // +1 for applications without group
		...
		GetGroupApplicationsRequest appsReq = new GetGroupApplicationsRequest(null);
		Utils.sign(appsReq, headers);
		List<ApplicationInfo> applications = applicationService.getGroupApplications(appsReq);
		if (!applications.isEmpty()) {
			TreeGroupRDTO noGroup = new TreeGroupRDTO();
			noGroup.setKey("no.group.applications");
			noGroup.setTranslationsGroup("appmanagement");
			noGroup.setApplications(new ArrayList<TreeApplicationRDTO>());
			generateTreeApplications(noGroup, applications);
			appGroups.add(noGroup);
		}

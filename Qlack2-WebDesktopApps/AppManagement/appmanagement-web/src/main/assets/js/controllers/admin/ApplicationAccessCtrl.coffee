angular
	...
	.controller 'ApplicationAccessCtrl', ['$scope', '$state', '$stateParams', 'WindowService', 'ConfigHttpService', '$q', '$timeout', \
	($scope, $state, $stateParams, WindowService, ConfigHttpService, $q, $timeout) ->
		...
		$scope.subjectsDataSource = new kendo.data.DataSource(
			transport:
				read: (options) ->
					$q.all([ConfigHttpService.getManagedUsers($stateParams.appId), ConfigHttpService.getManagedGroups($stateParams.appId)]).then(

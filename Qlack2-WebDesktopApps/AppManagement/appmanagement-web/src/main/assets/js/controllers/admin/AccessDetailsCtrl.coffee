angular
	...
	($scope, $state, $stateParams, ConfigHttpService, $window) ->
		...
		ConfigHttpService.getOperations($stateParams.appId, $stateParams.ownerId).then(
			success = (response) ->
...
#			Convert "null" returned by the html select to null before submitting the data
			...
			ConfigHttpService.saveOperations($stateParams.appId, $stateParams.ownerId, $scope.accessData).then(
				success = (response) ->
					NotificationSrv.add(
						title: "appmanagement.operations_saved_title"
						content: "appmanagement.operations_saved_content"
						content_data:

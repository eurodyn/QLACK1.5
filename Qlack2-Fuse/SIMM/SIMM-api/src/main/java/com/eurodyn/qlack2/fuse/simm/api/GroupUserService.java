...
public interface GroupUserService {
    ...
    /**
     * Joins the user in a group basically it creates mapping between Group and a user.
     * @param userId user ID
     * @param groupId group ID
     * @return
     * @throws QSIMMException Throws exception if user has already joined the group.
     */
     ...
     */
    SocialGroupDTO[] listGroupsForUser(String userID, String searchTerm, byte[] status, PagingParams paging) throws QSIMMException;

    /**
     * Changes status of user in group as accepted (GROUP_USER_STATUS_ACCEPTED = 1)
     * to join the group.
     * @param userID User ID
     * @param groupID group ID
    ...
    /**
     * Changes status of user in group as rejected to join the group
     * Following are the status of rejection:
     * GROUP_USER_STATUS_REJECTED_MODERATOR(3)- If rejected by the moderator to join the group
     * GROUP_USER_STATUS_REJECTED_USER(4) - If rejected by the user to join the group.
     * @param userID User ID
     * @param groupID group ID
     * @throws QSIMMException Throws exception if user has not been joined the group.
     */
    ...
    /**
     * Removes the user for provided group,
    ...
    /**
     * Returns a list with all the users which are members in the same groups
    ...
    /**
     * Returns array of available Groups as Groups DTOs for User to join for provided user ID.
     ...
     * @throws QSIMMException Throws exception if Provided userID is null.
     */
    public SocialGroupDTO[] listAvailableGroupsForJoining(String searchTerm, String userID, byte[] status, PagingParams paging) throws QSIMMException;

    /**
     * This method returns all group users for provided userID and status
     * for eg. status = 0(GROUP_USER_STATUS_REQUESTED_NEW) implies all the
     * group invitations to a user that are not yet responded by user.
     * @param userID
     * @param status
     ...
     *
     * @param userID
     * @param groupDTO The group to which the user is being invited. This method takes into
     * account the id property of the group object, as well as the srcUserId property in case
     * a JMS message is posted for this action
     */

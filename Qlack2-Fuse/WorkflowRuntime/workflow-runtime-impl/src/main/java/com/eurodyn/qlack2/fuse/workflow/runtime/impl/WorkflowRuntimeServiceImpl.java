...
public class WorkflowRuntimeServiceImpl implements WorkflowRuntimeService {
	...
	@Override
	public Long createWorkflowInstance(String processId, String content, Map<String, Object> parameters) {
		long processInstanceId = -1;	
		//change classloader due to mvel issue with classloaders in OSGi
		Thread thread = Thread.currentThread();
		ClassLoader loader = thread.getContextClassLoader();
		...
		{
			logger.log(Level.SEVERE, "createWorkflowInstance: Exception throws: " + e.toString());
			audit("LOG_EVENT_CREATE_WORKFLOW_INSTANCE", processId, null, e);
			throw new QWorkflowRuntimeException(e.toString(), e);
		}
		finally {
			// revert the classloader
	...
	@Override
	public Long startWorkflowInstance(Long processInstanceId) {
		//change classloader due to mvel issue with classloaders in OSGi
		Thread thread = Thread.currentThread();
		ClassLoader loader = thread.getContextClassLoader();	
		try {
		...
		{
			logger.log(Level.SEVERE, "startWorkflowInstance: Exception throws: " + e.toString());
			audit("LOG_EVENT_START_WORKFLOW_INSTANCE", processInstanceId.toString(), null, e);
			throw new QWorkflowRuntimeException(e.toString(), e);
		}
		finally {
			// revert the classloader
	...
	@Override
	public void signalWorkflowInstance(Long processInstanceId, String signalName, Object event)
	{
		ProcessInstance processInstance = null;
		//change classloader due to mvel issue with classloaders in OSGi
		Thread thread = Thread.currentThread();
    ...
    public TaskSummary getTaskDetails(Long processInstanceId, Long taskId)
    {
        ...
        if (task != null) {
            ...
            Date completionTime = getTaskEventCompletionbyTaskId(task.getId());
            return new TaskSummary(task.getId(), task.getNames().get(0).getText(),
                    ...
                    , task.getTaskData().getParentId(), completionTime, potOwnersList);
        }
        return null;
    }
        ...
        for (org.kie.api.task.model.TaskSummary myTask : myTasks)
        {
        	if (myTask.getProcessInstanceId().equals(processInstanceId))
        	{
	        	Task task = taskService.getTaskById(myTask.getId());
	             ...
	             Date completionTime = getTaskEventCompletionbyTaskId(myTask.getId());
	             taskSummaries.add(ConverterUtil.adapt(myTask, completionTime, potOwnersList));
        	}
        }
	...
	@Override
    public List<TaskSummary> getTasksByStatusByProcessInstanceId(Long processInstanceId, List<String> statusList) {
    	ProcessInstanceIdContext myContext = ProcessInstanceIdContext.get(processInstanceId);
        TaskService taskService = runtimeManager.getRuntimeEngine(myContext).getTaskService();
        List<Status> queryStatusList = new ArrayList<>();
		if ( statusList!= null )
    ...
    public String getTaskVariableValueByTaskId(Long taskId, String variableName) {
        ...
        return variableValue.size() > 0 ? variableValue.get(0) : null;
    }

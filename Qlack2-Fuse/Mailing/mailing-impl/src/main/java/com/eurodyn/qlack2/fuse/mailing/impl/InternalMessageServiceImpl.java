...
public class InternalMessageServiceImpl implements InternalMessageService {
	...
	private static final Logger LOGGER = Logger.getLogger(InternalMessageServiceImpl.class.getName());
	...
	public List<InternalMessagesDTO> getInternalInboxFolder(String userId) {
		...
		List<InternalMessagesDTO> dtoList = ConverterUtil.internalMessageConvertList(internalMessagesList);
	 ...
	 * <ul>
	 * <li>
	 * if the folder that contains the message is the inbox and the sender has
	 * already deleted the message, then the message is permanently removed from
	 * the system.</li>
	 * <li>
	 * if the folder that contains the message is the inbox and the sender has
	 * not deleted the message, then the message is marked as
	 * "deleted from the sender".</li>
	 * <li>
	 * if the folder that contains the message is the sent folder and the
	...
	public List<InternalAttachmentDTO> getInternalMessageAttachments(String messageId) {
		List<InternalAttachment> internalAttachments = InternalAttachment.findByMessagesId(em, messageId);

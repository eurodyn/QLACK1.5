...
public class AuditLoggingServiceImpl implements
		...
		if (searchList != null) {
			for (SearchDTO searchDTO : searchList) {
				if (SearchOperator.EQUAL == searchDTO.getOperator()) {
					...
					cq = applyLikeOperator(cb, root, cq, searchDTO);
				}
			}
		}
	...
	private <T> CriteriaQuery<T> applyLikeOperator(CriteriaBuilder cb, Root<Audit> root, CriteriaQuery<T> cq, SearchDTO searchDTO) {
		if ("levelId".equals(searchDTO.getColumn().name())) {
